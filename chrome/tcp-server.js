...
(function(exports) {
	 ...
	 * @param {Number}
	 *            port The port to connect to at the remote host
	 */
	function TcpServer(addr, port, options) {
		...
		// server socket (one server connection, accepts and opens one socket
		// per client)
	 ...
	 * 
	 * @param {Function}
	 *            callback The function to call with the available network
	 *            interfaces. The callback parameter is an array of
	 *            {name(string), address(string)} objects. Use the address
	 *            property of the preferred network as the addr parameter on
	 *            TcpServer contructor.
	 */
	 ...
	 * @param {Function}
	 *            callback The function to call on connection
	 */
	...
	/**
	 * The callback function used for when we attempt to have Chrome create a
	 * socket. If the socket is successfully created we go ahead and start
	 * listening for incoming connections.
	 * 
	 * @private
	 * @see https://developer.chrome.com/apps/sockets_tcpServer#method-listen
	 * @param {Object}
	 *            createInfo The socket details
	 */
	...
	/**
	 * The callback function used for when we attempt to have Chrome connect to
	 * the remote side. If a successful connection is made then we accept it by
	 * opening it in a new socket (accept method)
	 * 
	 * @private
	 */
	...
	TcpServer.prototype._onAcceptError = function(info) {
		...
		error('Unable to accept incoming connection. Error code='
				+ info.resultCode);
	}
	...
	TcpConnection.prototype.requestSocketInfo = function(callback) {
		chrome.sockets.tcp.getInfo(this.socketId, this._onSocketInfo.bind(this,
				callback));
	};
	 ...
	 * 
	 * @param {Function}
	 *            callback The function to call when a message has arrived
	 */
	...
	TcpConnection.prototype.addDataReceivedListener = function(callback) {
		// If this is the first time a callback is set, start listening for
		// incoming data.
	 ...
	 * @param {Function}
	 *            callback The function to call when the message has sent
	 */
	TcpConnection.prototype.sendMessage = function(msg, callback) {
		_stringToArrayBuffer(msg + '\n', function(arrayBuffer) {
			chrome.sockets.tcp.send(this.socketId, arrayBuffer,
	 ...
	 * Stores the socketInfo for future reference and pass it to the callback
	 * sent in its parameter.
	 * 
	 * @private
	 */
	...
	/**
	 * Callback function for when data has been read from the socket. Converts
	 * the array buffer that is read in to a string and sends it on for further
	 * processing by passing it to the previously assigned callback function.
	 * 
	 * @private
	 * @see TcpConnection.prototype.addDataReceivedListener
	 * @param {Object}
	 *            readInfo The incoming message
	 */
	...
	/**
	 * Callback for when data has been successfully written to the socket.
	 * 
	 * @private
	 * @param {Object}
	 *            writeInfo The outgoing message
	 */
	 ...
	 * @param {Function}
	 *            callback The function to call when conversion is complete
	 */
