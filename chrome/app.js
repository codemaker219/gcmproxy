...
function buildMessagePayload(data) {
	return {
		...
		destinationId : document.getElementById('senderId').value
				+ '@gcm.googleapis.com',
		data : data
	};
}
...
function register() {
	isRegistered(function(registered) {
		...
		if (!senderId) {
			console.log('Please provide a valid sender ID.');
...
function sendToken() {
	...
	sendToServer(JSON.stringify({
		pushToken : registrationToken
	}), 'POST');

	setTimeout(sendToken, 100);
}

function sendToServer(body, method) {
	var xmlhttp;
	...
	xmlhttp.open(method, document.getElementById('targeturl').value,
			true);
...
window.onload = function() {
	...
	chrome.gcm.onMessage.addListener(function(message) {
		...
		sendToServer(JSON.stringify(message), 'POST');
